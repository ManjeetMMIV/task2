/**
 * Compact, fast 2D and 3D Simplex noise generator with Fractal Brownian Motion (fBm).
 * Produces continuous organic values in [-1, 1] without external dependencies.
 */
...
/**
 * 3D Simplex Noise for smooth time-varying surfaces (x, y, z/time)
 */
...
/**
 * Multi-octave Fractal Brownian Motion (fBm) for rich turbulent smoke textures
 */
export function fbm3D(x: number, y: number, z: number, octaves = 3): number {
  let value = 0
  let amplitude = 0.5
  let frequency = 1.0
